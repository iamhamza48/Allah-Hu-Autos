...
const BranchCard = ({ b }: { b: typeof BRANCHES[0] }) => {
  ...
  return (
    <div className="w-full rounded-xl border border-border overflow-hidden bg-card mb-3 shadow-sm">
      <button
        type="button"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground truncate">{b.addr.split(',')[0]}</p>
        </div>
        ...
        <div className="border-t border-border">
          <div className="relative w-full" style={{ height: '160px' }}>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="absolute bottom-2 right-2 z-10 flex items-center gap-1 bg-card/95 backdrop-blur border border-border text-muted-foreground hover:text-primary text-[10px] px-2 py-1 rounded-full transition-colors"
            >
          ...
          <div className="px-3 py-3 space-y-2 bg-muted/30">
            ...
            <div className="flex gap-2 text-xs text-muted-foreground">
              ...
              <a href={`tel:${b.phone.replace(/\s/g,'')}`} className="hover:text-primary transition-colors">{b.phone}</a>
            </div>
            <div className="flex gap-2 text-xs text-muted-foreground">
...
const Footer = () => (
  <footer className="w-full bg-brand-slate text-brand-muted border-t border-border">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-6 items-start">
        ...
        <div className="sm:col-span-2 lg:col-span-4 lg:pr-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h3 className="font-black text-foreground text-lg leading-tight">Allah-Hu-Autos</h3>
              <p className="text-[10px] text-primary font-bold uppercase tracking-widest mt-0.5">We Take Pride in Your Ride</p>
            </div>
          </div>
          ...
          <a href="mailto:info@allahhuautos.pk" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-background/20 border border-border">
        ...
        {LINK_SECTIONS.map((section) => (
          <div key={section.title} className="lg:col-span-2">
            <h4 className="font-bold text-foreground text-sm mb-4">{section.title}</h4>
            <ul className="space-y-2.5">
              {section.links.map(([path, label]) => (
                <li key={path}>
                  <Link to={path} className="text-sm text-muted-foreground hover:text-primary flex items-center gap-2 group transition-colors">
        ...
        <div className="sm:col-span-2 lg:col-span-4">
          <div className="mb-4">
            <h4 className="font-bold text-foreground text-sm mb-1">Our Branches</h4>
            <p className="text-xs text-muted-foreground">Click a branch to view location</p>
          </div>
    ...
    <div className="border-t border-border bg-black/20">
      <div className="container px-4 md:px-6 mx-auto flex flex-col sm:flex-row py-4 justify-between gap-2 text-xs text-muted-foreground">
        <p>© {new Date().getFullYear()} Allah-Hu-Autos. All rights reserved.</p>
        <p className="flex items-center gap-1">Made with <span className="text-primary mx-1">♥</span> in Pakistan</p>
      </div>
    </div>
  </footer>
);
