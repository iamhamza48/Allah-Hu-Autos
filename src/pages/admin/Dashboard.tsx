    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {statCards.map(({ icon: Icon, label, value, iconClass, accent, note, wide }) => (
          <Card key={label} className={`border-l-4 ${accent} ${wide ? 'sm:col-span-2 lg:col-span-4' : ''} shadow-sm bg-card border border-border`}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {note && <p className="text-[11px] text-primary font-semibold mt-0.5">{note}</p>}
                </div>
                <Icon className={`h-8 w-8 ${iconClass} opacity-80`} />
              </div>
            </CardContent>
          </Card>
        ))}
              ...
              recentOrders.map(o => (
                <div key={o.id} className="flex items-center justify-between px-4 py-3">
                  ...
                  <div className="text-right flex flex-col items-end gap-1">
                    <p className="text-sm font-semibold text-foreground">{formatPKR(o.total)}</p>
            ...
            {loading ? (
              ...
              recentBookings.map(b => (
                <div key={b.id} className="flex items-center justify-between px-4 py-3">
                  ...
                  <div className="text-right flex flex-col items-end gap-1">
                    <p className="text-[11px] text-muted-foreground">{b.branch?.name || '—'}</p>
